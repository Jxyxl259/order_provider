(function($) {
    ...
    $.Autocompleter = function($elem, options) {
        ...
        if (isNaN(this.options.minChars)) {
		...
		// choose button
		var btnChoose = $elem.siblings('span.choose');
		if(btnChoose.length){
			btnChoose.click(function(e){
				e.preventDefault();

				self.disableFinishOnBlur();
        ...
        $elem.keyup(function(e) {
            ...
            switch(self.lastKeyPressed_) {
                ...
                default:
									if(self.options.filterResults || !btnChoose.length){
										self.activate();
									}
            }
        });
				$elem.focus(function(e) {
						if(!self.active_ && (self.options.filterResults || !btnChoose.length)){
							self.activate();
						}
				});
				$elem.bind('ac-finish', function(e){
					e.preventDefault();
					e.stopPropagation();
					self.finish();
				});
     ...
     */
    $.Autocompleter.prototype.activate = function(force) {
		this.activateNow(force);
    };

    /**
     * Activate autocompleter immediately
     */
    $.Autocompleter.prototype.activateNow = function(force) {
        var value = this.beforeUseConverter(this.dom.$elem.val());
        if (force || (value !== this.lastProcessedValue_ && value !== this.lastSelectedValue_)) {
    ...
    $.Autocompleter.prototype.fetchData = function(value) {
		var flag = true;
		if(this.options.fetchRemoteDataFilter){
			flag = this.options.fetchRemoteDataFilter(value);
		}
		if(!flag){
			this.finish();
			return;
        ...
        if (this.options.data) {
            ...
            this.filterAndShowResults(this.options.getData(value), value);
		} else {
			var self = this;
    ...
    $.Autocompleter.prototype.filterAndShowResults = function(results, filter) {
		if(this.options.processData){
			results = this.options.processData(results);
		}
        ...
        for (i = 0; i < results.length; i++) {
            ...
            if (value > '') {
                ...
                if (this.options.filterResults) {
                    ...
                    if (this.options.matchLabel) {
                        testValue = this.showResult(value, data);
                    }
    ...
    $.Autocompleter.prototype.selectCurrent = function() {
        var $item = $('li', this.dom.$results);
		var $itemSelected = $item.filter('.' + this.selectClass_);
        if ($itemSelected.length === 1) {
            this.selectItem($itemSelected);
        } else {
			if ($item.length) {
				this.selectItem($item.eq(0));
			}else{
				this.finish();
			}
        }

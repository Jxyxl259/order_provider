$(document).ready(function(){
	...
	if( $('#demo-d3-heatmap').length > 0 ) {
		...
		var dataHeat = "common/js/plugins/stat/data-heatmap.tsv"; // path/to/your/datafile.tsv
		...
		d3.tsv(dataHeat,
			...
			function(error, data) {
				...
				legend.append("text")
					.attr("class", "mono")
					.text(function(d) { return "鈮�" + Math.round(d); })
					.attr("x", function(d, i) { return legendElementWidth * i; })
	...
	if( $('#demo-d3-barchart').length > 0 ) {
		var dataBar = "common/js/plugins/stat/data-barchart.tsv"; // path/to/your/datafile.tsv
